...
if [ "$TRAVIS_PULL_REQUEST" != "false" ]; then
	git checkout master
	ssh-agent bash -c 'ssh-add $TRAVIS_BUILD_DIR/.travis/id; git fetch origin "pull/$TRAVIS_PULL_REQUEST/head:pr-$TRAVIS_PULL_REQUEST"'
	git merge --no-ff -m "Merge pull request #$TRAVIS_PULL_REQUEST" "pr-$TRAVIS_PULL_REQUEST"
